...
// Lista de canciones locales (puedes agregar más archivos .txt aquí fácilmente)
const listaCanciones = [
    { archivo: "Dios_esta_aqui.txt", titulo: "Dios está aquí", artista: "alabanza" },
    { archivo: "quiero_decir.txt", titulo: "Quiero Decir que Sí", artista: "Canto a Maria" },
    { archivo: "carros_del_faraon.txt", titulo: "Carros del Faraón", artista: "alabanza" },
    { archivo: "De_aqui_hasta_el_final.txt", titulo: "De aqui hasta el final", artista: "Bodas" },
    { archivo: "Gozate.txt", titulo: "Gozate", artista: "alabanza" },
];

let textoOriginal = "";
let semitonosActuales = 0;  

// Escala musical para transponer tonos
...
function actualizarVisualizacion() {
    ...
    // Aplicar transposición si hay cambios de tono
...
// Cargar opciones en el menú desplegable al iniciar
...
selector.addEventListener('change', async (e) => {
    ...
    const info = listaCanciones.find(c => c.archivo === archivo);
    semitonosActuales = 0;
    ...
    if (info) {
        ...
        try {
            const respuesta = await fetch(`letras/${archivo}`);
            if (!respuesta.ok) throw new Error("No se pudo cargar el archivo");
            textoOriginal = await respuesta.text();
            actualizarVisualizacion();
        } catch (error) {
            letra.innerText = "Error al cargar la letra del archivo.";
            textoOriginal = "";
        }
    } else {
...
// Botones de transposición e impresión
